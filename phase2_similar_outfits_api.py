# Phase 2: Similar Outfits API (On-demand)
# This generates 10 similar outfits for any given outfit in real-time
...
class SimilarOutfitsGenerator:
    """
    Phase 2: Generate 10 similar outfits for any given outfit on-demand
    """
    
    def __init__(self, config: Dict = None):
        """Initialize the similar outfits generator."""
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            'user_outfits_dir': "data/user_recommendations",
            'batch_size': 50
        }
    ...
    def load_outfit_from_csv(self, main_outfit_id: str) -> Dict:
        """Load outfit data from pre-computed CSV files."""
        try:
            parts = main_outfit_id.split('_')
            if len(parts) >= 3 and parts[0] == 'main':
                user_id = parts[1]
                
                # Load user's main outfits CSV
                csv_file = os.path.join(self.config['user_outfits_dir'], f"user_{user_id}_main_outfits.csv")
                
                if not os.path.exists(csv_file):
                    raise FileNotFoundError(f"User outfits file not found: {csv_file}")
                
                df = pd.read_csv(csv_file)
                outfit_row = df[df['main_outfit_id'] == main_outfit_id]
                
                if outfit_row.empty:
                    raise ValueError(f"Outfit {main_outfit_id} not found in user {user_id} outfits")
                
                return outfit_row.iloc[0].to_dict()
            else:
                raise ValueError(f"Invalid main_outfit_id format: {main_outfit_id}")
                
        except Exception as e:
            logger.error(f"Error loading outfit {main_outfit_id}: {e}")
            raise
    ...
    def find_similar_outfits(self, outfit_id: str, num_similar: int = 10) -> List[Dict]:
        ...
        try:
            # Load source outfit
            source_outfit = self.load_outfit_from_csv(outfit_id)
