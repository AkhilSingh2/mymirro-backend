# Phase 3 ENHANCED: Same-Category Similar Products with User Preferences
# Focus: Same product type with color/design diversity + user preference integration
...
class EnhancedSimilarProductsGenerator:
    """
    Phase 3 ENHANCED: Same-category similar products with diversity and user preferences
    """
    
    def __init__(self, config: Dict = None):
        """Initialize the enhanced similar products generator."""
        ...
        # Load price data
        self.price_mappings = self._load_price_mappings()
        
        # Load product context map for outfit intelligence
        self.context_mappings = self._load_context_mappings()
        
    def _default_config(self) -> Dict:
        """Enhanced default configuration for same-category focus."""
        return {
            'model_name': "all-MiniLM-L6-v2",
            'cache_embeddings': True,
            'products_file': "data/local_processed_results_enhanced.csv",
            'price_file': "data/products_scraped.csv",
            'context_file': "data/user_recommendations/product_outfit_context.csv",  # NEW: Context map
            'batch_size': 50,
    ...
    def _load_price_mappings(self) -> Dict:
        """Load actual price data from scraped results."""
        try:
            if os.path.exists(self.config['price_file']):
                price_df = pd.read_csv(self.config['price_file'])
                logger.info(f"✅ Loaded {len(price_df)} price mappings from scraped data")
                ...
                for _, row in price_df.iterrows():
                    ...
                    if product_id and price:
                        ...
                        if isinstance(price, str):
                            price = float(''.join(filter(str.isdigit, price))) or 1000
                        price_dict[product_id] = max(float(price), 500)  # Minimum ₹500
                
                logger.info(f"✅ Processed {len(price_dict)} valid price mappings")
                return price_dict
            else:
                logger.warning(f"Price file not found: {self.config['price_file']}")
                return {}
        except Exception as e:
            logger.error(f"Error loading price mappings: {e}")
            return {}
    
    def _load_context_mappings(self) -> Dict:
        """Load pre-computed product-outfit context mappings for intelligent recommendations."""
        try:
            if os.path.exists(self.config['context_file']):
                context_df = pd.read_csv(self.config['context_file'])
                logger.info(f"✅ Loaded {len(context_df)} product context mappings from Phase 1 data")
                
                context_dict = {}
                for _, row in context_df.iterrows():
                    product_id = str(row.get('product_id', ''))
                    if product_id:
                        compatible_products = row.get('compatible_products', '')
                        if compatible_products and isinstance(compatible_products, str):
                            compatible_list = [p.strip() for p in compatible_products.split(',') if p.strip()]
                        else:
                            compatible_list = []
                        
                        context_dict[product_id] = {
                            'outfit_contexts': row.get('outfit_contexts', ''),
                            'compatible_products': compatible_list,
                            'style_context': row.get('style_context', ''),
                            'occasion_context': row.get('occasion_context', ''),
                            'formality_level': row.get('formality_level', 5.0),
                            'price_range': row.get('price_range', 'Mid-Range'),
                            'compatibility_score': row.get('compatibility_score', 0.5)
                        }
                
                logger.info(f"✅ Processed {len(context_dict)} valid context mappings")
                return context_dict
            else:
                logger.warning(f"Context file not found: {self.config['context_file']}")
                logger.info("Phase 3 will work without outfit context intelligence")
                return {}
        except Exception as e:
            logger.error(f"Error loading context mappings: {e}")
            return {}
    ...
    def validate_products_data(self, products_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Integrate actual prices
        if self.price_mappings:
            products_df['actual_price'] = products_df.apply(
                lambda row: self.price_mappings.get(
                    str(row.get('product_id', '')), 
                    self._estimate_price_from_title(row.get('title', ''))
                ), axis=1
            )
        else:
            products_df['actual_price'] = products_df.apply(
        ...
        logger.info(f"✅ Products validation complete. Price range: ₹{products_df['actual_price'].min():.0f} - ₹{products_df['actual_price'].max():.0f}")
    ...
    def find_similar_products(self, product_id: str, num_similar: int = 10, 
        ...
        """Enhanced same-category similar products with diversity."""
    ...
    def _generate_same_category_candidates(self, source_product: pd.Series, products_df: pd.DataFrame, 
        ...
        # 4. Outfit context intelligent candidates (NEW: Uses Phase 1 intelligence)
        if self.config['enable_context_intelligence'] and self.context_mappings:
            context_candidates = self._get_outfit_context_candidates(
                source_product, products_df, user_preferences
            )
            all_candidates.extend(context_candidates)
        
        # 5. User preference enhanced candidates (same category)
        if user_preferences:
    ...
    def _get_outfit_context_candidates(self, source_product: pd.Series, products_df: pd.DataFrame, 
                                     user_preferences: Dict = None) -> List[Dict]:
        """Get candidates based on pre-computed outfit context intelligence (Fast!)."""
        
        candidates = []
        source_product_id = str(source_product.get('product_id', ''))
        source_wear_type = source_product.get('wear_type', '')
        
        # Fast lookup in pre-computed context map
        if source_product_id in self.context_mappings:
            context_info = self.context_mappings[source_product_id]
            compatible_products = context_info['compatible_products']
            
            logger.info(f"🎯 Found {len(compatible_products)} outfit-context compatible products for {source_product_id}")
            
            # Convert compatible product IDs to actual candidates
            for compatible_id in compatible_products[:12]:  # Top 12 context-based
                # Search for this specific product in same category
                product_query = f"product_id:{compatible_id} {source_wear_type}"
                context_candidates = self.search_similar_products_faiss(
                    product_query, source_wear_type, k=3
                )
                
                for candidate in context_candidates:
                    candidate_id = str(candidate['product'].get('product_id', ''))
                    if candidate_id == compatible_id:  # Exact match
                        candidate['candidate_type'] = 'outfit_context'
                        candidate['boost_factor'] = 1.25  # Strong boost for outfit intelligence
                        candidate['context_score'] = context_info['compatibility_score']
                        candidates.append(candidate)
                        break
        
        return candidates[:8]  # Limit outfit context candidates
    ...
    def _get_user_preference_candidates(self, source_product: pd.Series, products_df: pd.DataFrame, 
        ...
        # Match with lower/upper wear captions if available
        if 'upper_wear_caption' in user_preferences or 'lower_wear_caption' in user_preferences:
            caption_candidates = self._get_wear_caption_candidates(
                source_product, products_df, user_preferences
            )
    ...
    def _get_wear_caption_candidates(self, source_product: pd.Series, products_df: pd.DataFrame, 
                                   user_preferences: Dict) -> List[Dict]:
        """Get candidates based on user's wear caption preferences."""
        
        candidates = []
        source_wear_type = source_product.get('wear_type', '')
        
        # Match with upper wear caption
        if source_wear_type == 'Upperwear' and 'upper_wear_caption' in user_preferences:
            upper_caption = user_preferences['upper_wear_caption'].lower()
            caption_candidates = self.search_similar_products_faiss(
                upper_caption, source_wear_type, k=8
            )
            for candidate in caption_candidates:
                candidate['candidate_type'] = 'wear_caption_match'
                candidate['boost_factor'] = 1.2  # Strong boost for caption matching
            candidates.extend(caption_candidates)
        
        # Match with lower wear caption
        elif source_wear_type == 'Bottomwear' and 'lower_wear_caption' in user_preferences:
            lower_caption = user_preferences['lower_wear_caption'].lower()
            caption_candidates = self.search_similar_products_faiss(
                lower_caption, source_wear_type, k=8
            )
            for candidate in caption_candidates:
                candidate['candidate_type'] = 'wear_caption_match'
                candidate['boost_factor'] = 1.2
            candidates.extend(caption_candidates)
    ...
    def _score_and_filter_same_category_candidates(self, source_product: pd.Series, candidates: List[Dict], 
        ...
        for candidate in candidates:
            ...
            similar_products.append({
                ...
                'price': candidate_product.get('actual_price', 1000),
    ...
    def _calculate_enhanced_same_category_similarity(self, source_product: pd.Series, candidate_product: pd.Series, 
        ...
        price_score = self._calculate_price_similarity_enhanced(
            source_product.get('actual_price', 1000),
            candidate_product.get('actual_price', 1000)
        )
        ...
        # Wear caption matching boost
        caption_score = self._calculate_wear_caption_similarity(
            source_product, candidate_product, user_preferences
        )
        
        # Outfit context intelligence score (NEW: Phase 1 derived)
        ...
        final_score = (
            ...
            context_score * weights['outfit_context_weight'] +  # NEW: Context intelligence
            price_score * weights['price_weight'] +
            preference_score * weights['user_preference_weight'] +
            caption_score * weights['wear_caption_weight']
        ) * type_boost
        ...
        total_weight = (weights['semantic_weight'] + weights['style_weight'] + 
                       ...
                       weights['outfit_context_weight'] + weights['price_weight'] + 
                       weights['user_preference_weight'] + weights['wear_caption_weight'])
        ...
        score_breakdown = {
            ...
            'outfit_context_intelligence': context_score,  # NEW: Phase 1 context
            'price_similarity': price_score,
            'user_preference_boost': preference_score,
            'wear_caption_match': caption_score,
    ...
    def _calculate_user_preference_boost(self, product: pd.Series, user_preferences: Dict) -> float:
        ...
        if preferred_price_range:
            ...
            product_price = product.get('actual_price', 1000)
            if min_price <= product_price <= max_price:
                boost += 0.1
        
        return min(boost, 1.4)  # Cap at 1.4x boost
    
    def _calculate_wear_caption_similarity(self, source_product: pd.Series, candidate_product: pd.Series, 
                                         user_preferences: Dict = None) -> float:
        """Calculate similarity based on wear caption matching."""
        if not user_preferences:
            return 0.5
        
        score = 0.5  # Default
        
        # Check if source and candidate are both upper wear
        if (source_product.get('wear_type', '') == 'Upperwear' and 
            candidate_product.get('wear_type', '') == 'Upperwear' and
            'upper_wear_caption' in user_preferences):
            
            upper_caption = user_preferences['upper_wear_caption'].lower()
            candidate_title = candidate_product.get('title', '').lower()
            
            # Simple keyword matching
            caption_words = set(upper_caption.split())
            title_words = set(candidate_title.split())
            overlap = len(caption_words.intersection(title_words))
            
            if overlap > 0:
                score = 0.7 + min(overlap * 0.1, 0.3)  # 0.7 to 1.0 range
        
        # Check if source and candidate are both lower wear
        elif (source_product.get('wear_type', '') == 'Bottomwear' and 
              candidate_product.get('wear_type', '') == 'Bottomwear' and
              'lower_wear_caption' in user_preferences):
            
            lower_caption = user_preferences['lower_wear_caption'].lower()
            candidate_title = candidate_product.get('title', '').lower()
            
            caption_words = set(lower_caption.split())
            title_words = set(candidate_title.split())
            overlap = len(caption_words.intersection(title_words))
            
            if overlap > 0:
                score = 0.7 + min(overlap * 0.1, 0.3)
...
def main():
    """Test the enhanced same-category similar products generator."""
    
    # Initialize enhanced generator
    generator = EnhancedSimilarProductsGenerator()
    
    # Sample user preferences with wear captions
    user_preferences = {
        'preferred_styles': ['Business Formal', 'Formal'],
        'preferred_colors': ['Black', 'Navy', 'White'],
        'price_range': (800, 2500),
        'upper_wear_caption': 'formal business blazer professional',
        'lower_wear_caption': 'formal trousers dress pants'
    }
    ...
    test_product_id = "5438"
    
    logger.info(f"🔍 Testing Enhanced Same-Category Phase 3 with product: {test_product_id}")
    ...
    try:
        ...
        if similar_products:
            ...
            print(f"\n🔗 Enhanced Same-Category API: GET /api/product/{test_product_id}/similar?count=8&diverse=true&personalized=true")
        else:
